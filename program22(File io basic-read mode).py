...
unlike other languages Python has provided its users a variety of options.
We will discuss seven of them in this tutorial.
...
t : t mode is used to open our file in text mode and only proper text files can be opened by it.
    It deals with the file data as a string.(default)
...
"""
#Read function
f = open("file.txt","rt")#open(file name,mode) return file pointer
content = f.read()
print(content)
f.close()
#line by line read function
f = open("file.txt","rt")
for line in f:
    print(line,end="")
f.close()
#Readline/readlines function(as like ists)
f = open("file.txt","rt")#open(file name,mode) return file pointer
print(f.readline())
print(f.readlines())
f.close()
